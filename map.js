...
lyrElement.className = 'layerButtonDiv';
...
zoomInInteraction.on('boxend', function () {
    ...
    map.getView().fit(zoomInExtent, map.getSize());
});
...
function legend() {
    ...
    overlays.getLayers().getArray().slice().forEach(layer => {
        ...
        element.appendChild(head);
        var img = new Image();
        img.src = "http://localhost:8081/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" + layer.get('title');
...
// add draw interaction for special query
...
function addInteractionForSpatialQuery(intType) {
    ...
    draw.on('drawend', function (e) {
        ...
        console.log(markerFeature.getGeometry());
...
$(function() {
    ...
    document.getElementById("srcCriteria").onchange = function () {
        ...
        if (clickSelectedFeatureOverlay) {
            clickSelectedFeatureOverlay.setStyle();
            clickSelectedFeatureOverlay = undefined;
        }
    ...
    document.getElementById('spQryRun').onclick = function () {
        ...
        if (markerFeature) {
            ...
            if (value_attribute == 'Within Distance of') {
                ...
                url = "http://localhost:8081/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=DWITHIN(geom," + markerType + "(" + coordList + ")," + value_dist + "," + value_distanceUnit + ")&outputFormat=application/json";


            } else if (value_attribute == 'Intersecting') {
                url = "http://localhost:8081/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=INTERSECTS(geom," + markerType + "(" + coordList + "))&outputFormat=application/json";
            } else if (value_attribute == 'Completely Within') {
                url = "http://localhost:8081/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=WITHIN(geom," + markerType + "(" + coordList + "))&outputFormat=application/json";
            }
...
function query() {
    ...
    var url = "http://localhost:8081/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "%20" + value_operator + "%20" + value_txt + "&outputFormat=application/json"
    //console.log(url);
...
function wms_layers() {
    ...
    fetch('getcapabilities_1.1.1.xml').then(function(response) {
        console.log("in func");
...
function info() {
    if (document.getElementById("info_btn").innerHTML == "☰ Activate GetInfo") {
        ...
        document.getElementById("info_btn").setAttribute("class", "btn btn-secondary btn-sm");
        if (popup) {
            popup.hide();
        }
    }
...
function clear_all() {
    ...
    document.getElementById("query_panel_btn").setAttribute("class", "btn btn-secondary btn-sm");
...
function show_hide_querypanel() {
    ...
    if (document.getElementById("query_tab").style.visibility == "hidden") {
        ...
        document.getElementById("query_panel_btn").style.left = "25%";
        document.getElementById("wms_layers_btn").style.left = "39%";
        document.getElementById("clear_btn").style.left = "55%";
        document.getElementById("measuretype").style.left = "65%";
        document.getElementById("info_btn").style.left = "85%";
        ...
        document.getElementById("query_panel_btn").innerHTML = "☰ Open Query Panel";
        document.getElementById("query_panel_btn").setAttribute("class", "btn btn-secondary btn-sm");
        document.getElementById("query_panel_btn").style.left = "10%";
        document.getElementById("wms_layers_btn").style.left = "30%";
        document.getElementById("clear_btn").style.left = "50%";
        document.getElementById("measuretype").style.left = "60%";
        document.getElementById("info_btn").style.left = "80%";
        document.getElementById("query_tab").style.width = "0%";
        document.getElementById("map").style.width = "100%";
        document.getElementById("map").style.left = "0%";
        document.getElementById("query_tab").style.visibility = "hidden";
...
function show_hide_legend() {
    ...
    if (document.getElementById("legend").style.visibility == "hidden") {
	    ...
	    document.getElementById("legend_btn").setAttribute("class", "btn btn-secondary btn-sm");
        document.getElementById("legend_btn").innerHTML = "☰ Show Legend";
        document.getElementById("legend").style.width = "0%";
        document.getElementById("legend").style.visibility = "hidden";
        document.getElementById('legend').style.height = '0%';
    ...
    if (value !== 'None') {
		...
		draw1 = new ol.interaction.Draw({
            ...
            draw1.on('drawend', function(evt) {
                ...
                var url = "http://localhost:8081/geoserver/wfs?request=GetFeature&version=1.0.0&typeName=" + value_layer + "&outputFormat=json&cql_filter=INTERSECTS(the_geom," + wkt + ")";
                //alert(url);
